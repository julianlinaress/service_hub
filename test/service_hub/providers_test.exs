defmodule ServiceHub.ProvidersTest do
  ...
  describe "provider_types" do
    ...
    test "list_provider_types/1 returns all scoped provider_types" do
      scope = user_scope_fixture()
      other_scope = user_scope_fixture()
      provider_type = provider_type_fixture(scope)
      other_provider_type = provider_type_fixture(other_scope)
      assert Providers.list_provider_types(scope) == [provider_type]
      assert Providers.list_provider_types(other_scope) == [other_provider_type]
    end
    ...
    test "get_provider_type!/2 returns the provider_type with given id" do
      ...
      assert_raise Ecto.NoResultsError, fn ->
        Providers.get_provider_type!(other_scope, provider_type.id)
      end
    end
    ...
    test "create_provider_type/2 with valid data creates a provider_type" do
      ...
      assert provider_type.user_id == scope.user.id
    end
    ...
    test "update_provider_type/3 with invalid scope raises" do
      scope = user_scope_fixture()
      other_scope = user_scope_fixture()
      provider_type = provider_type_fixture(scope)

      assert_raise MatchError, fn ->
        Providers.update_provider_type(other_scope, provider_type, %{})
      end
    end
    ...
    test "delete_provider_type/2 with invalid scope raises" do
      scope = user_scope_fixture()
      other_scope = user_scope_fixture()
      provider_type = provider_type_fixture(scope)

      assert_raise MatchError, fn ->
        Providers.delete_provider_type(other_scope, provider_type)
      end
    end
  ...
  describe "auth_types" do
    ...
    test "list_auth_types/1 returns all scoped auth_types" do
      scope = user_scope_fixture()
      other_scope = user_scope_fixture()
      auth_type = auth_type_fixture(scope)
      other_auth_type = auth_type_fixture(other_scope)
      assert Providers.list_auth_types(scope) == [auth_type]
      assert Providers.list_auth_types(other_scope) == [other_auth_type]
    end
    ...
    test "get_auth_type!/2 returns the auth_type with given id" do
      ...
      assert_raise Ecto.NoResultsError, fn ->
        Providers.get_auth_type!(other_scope, auth_type.id)
      end
    end
    ...
    test "create_auth_type/2 with valid data creates a auth_type" do
      ...
      assert auth_type.name == "some name"
      assert auth_type.key == "some key"
      assert auth_type.required_fields == %{}
      assert auth_type.user_id == scope.user.id
    end
    ...
    test "update_auth_type/3 with valid data updates the auth_type" do
      ...
      assert auth_type.name == "some updated name"
      assert auth_type.key == "some updated key"
      assert auth_type.required_fields == %{}
    end

    test "update_auth_type/3 with invalid scope raises" do
      scope = user_scope_fixture()
      other_scope = user_scope_fixture()
      auth_type = auth_type_fixture(scope)

      assert_raise MatchError, fn ->
        Providers.update_auth_type(other_scope, auth_type, %{})
      end
    end
    ...
    test "delete_auth_type/2 with invalid scope raises" do
      scope = user_scope_fixture()
      other_scope = user_scope_fixture()
      auth_type = auth_type_fixture(scope)
      assert_raise MatchError, fn -> Providers.delete_auth_type(other_scope, auth_type) end
    end
